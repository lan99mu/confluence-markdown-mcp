...
`serve` 子命令只支持 MCP stdio，不再暴露 MCP SSE / streamable-http
传输。如果需要远程或容器内 HTTP 访问，请使用下面的纯 HTTP API
（`serve-http`），而不是 MCP 自身的 HTTP 传输。

#### 使用 Docker 运行

仓库提供了 `Dockerfile`，默认启动纯 HTTP API 并监听 `0.0.0.0:8000`。

```bash
# 构建镜像
docker build -t confluence-markdown-mcp .

# 以纯 HTTP API 模式启动（默认 CMD）
